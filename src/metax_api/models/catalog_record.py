...
class CatalogRecord(Common):
    ...
    def _get_top_level_parent_dirs_by_project(self, dir_identifiers):
        ...
        for dr in dirs:
            # excluding from code as new requirements do not demand this feature
    ...
    def calculate_directory_byte_sizes_and_file_counts(self):
        ...
        if self.research_dataset.get('files', None):
            file_dir_identifiers = [File.objects.get(identifier=i['identifier']).parent_directory.identifier
                for i in self.research_dataset['files']]
