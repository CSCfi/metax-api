...
class FileApiWriteRestoreTests(FileApiWriteCommon):
    ...
    def test_restore_files_ok(self):
        ...
        response = self.client.post('/rpc/files/restore', [f['identifier'] for f in deleted_files], format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_restore_files_recreate_missing_directories(self):
        ...
        response = self.client.post('/rpc/files/restore', file_identifiers, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual('restored_files_count' in response.data, True, response.data)
    ...
    def test_check_parameter_is_string_list(self):
        response = self.client.post('/rpc/files/restore', ['a', 'b', 1], format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_check_files_belong_to_one_project(self):
        ...
        response = self.client.post('/rpc/files/restore', [ f1.identifier, f2.identifier ], format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
